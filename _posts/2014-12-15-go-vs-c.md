...
In any case, my hope is that it is an enlightening nonetheless.
...
languages, like C. In Debian’s Computer Language Benchmarks Game, in
their “Ten Tiny Examples,” Go rates at 8^th^ on a scale comparing
(program time / fastest program time), coming in at the same order of
...
worth mentioning, however, that Debian does disclaim that language
